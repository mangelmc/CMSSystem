...
Template.sitios.events({
	...
	'submit #formsitio': function (e) {
		...
		if (formSitio.carrera.get() == false || formSitio.titulo.get() == false || formSitio.admin.get() == false) {
			alerta('Arregla los errores', 'warning');
			return;
		}

		var titulo = e.target.titulo.value.trim().split(" ").join("-");
		let sitio = SITIO.findOne({
			titulo: titulo
		});
		if (sitio != undefined) {
			alerta("El dominio '" + titulo + "' Ya existe..!", 'warning', 'Pruebe con otro dominio o Carrera');
			return;
		}
		var obj = {
			titulo: titulo,
...
Template.createuser.events({
	...
	'submit #regAdminForm': function (e) {
		...
		if (userForm.get() == false || emailForm.get() == false || nameForm.get() == false || surnameForm.get() == false || careerForm.get() == false || passForm.get() == false) {
			alerta('Debe Arreglar los errores del Formulario', 'error');
			return;
		}
		...
		Meteor.call('createNewUser', user, rol, function (error, result) {
			if (error) {
				alerta(error.reason, 'error');
				return false;
			}
			if (result) {
...
Template.sitioslist.events({
	...
	'change #useradmin': function (e) {
		var useradmin = Accounts.users.findOne({
			_id: e.target.value
		});
	...
	'submit #changeadminform': function (e) {
		...
		Meteor.call('changeAdmin', sitio, set, function (error, result) {
			if (result) {
				console.log(result);
				sAlert.success('Administrador modificado', {
					effect: 'slide',
					offset: '130'
				});
			}
